...
import com.beacon.operationlog.common.pojo.AbstractResultWrapper;
...
import com.beacon.operationlog.start.support.parse.LogRecordValueParser;
import com.beacon.operationlog.common.ThrowableUtil;
...
public class LogRecordInterceptor extends LogRecordValueParser implements InitializingBean, MethodInterceptor, Serializable {
    ...
    private Object execute(MethodInvocation invoker, Object target, Method method, Object[] args) throws Throwable {
        ...
        try {
            ...
            //记录日志错误不要影响业务
            log.error("log record parse exception:{}", ThrowableUtil.getStackTrace(t));
        } finally {
...
//            }
        ...
        String testParse = logRecord.getTestParse();
        ...
        if (logConditionPassed(logRecord.getCondition(), expressionValues)) {
            ...
            logRecord.setTestParse(expressionValues.get(testParse));
            ...
            try {
                ...
                if (bizLogService == null) {
                    log.error("bizLogService not init!!");
                }
                logRecord.setTime(System.currentTimeMillis() - currentTime.get());
                currentTime.remove();
                bizLogService.record(logRecord);
            } catch (Exception t) {
                log.error("log record execute exception:{}", ThrowableUtil.getStackTrace(t));
            }
        }
    }
    ...
    private List<String> getSpElTemplates(LogRecord logRecord) {
        List<String> spElTemplates = CollUtil.newArrayList(logRecord.getBizKey(), logRecord.getBizNo(), logRecord.getValue(), logRecord.getDetail(), logRecord.getTestParse());
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if (bizLogService == null) {
            log.error("bizLogService not null");
        }
    }
...
//            }
//        } catch (Exception e) {
//            log.error("setLogRecordHttpRequest:{}", ThrowableUtil.getStackTrace(e));
//        }
//    }
